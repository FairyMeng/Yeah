...
from Yeah_Rec.models.base import get_flags, flag_defaults, init_model
from Yeah_Rec.data.load_kg_rating_data import load_data
from Yeah_Rec.utils.trainer import ModelTrainer
from Yeah_Rec.utils.misc import evalRecProcess, evalKGProcess, to_gpu
from Yeah_Rec.utils.loss import bprLoss, orthogonalLoss, normLoss
from Yeah_Rec.utils.visuliazer import Visualizer
from Yeah_Rec.utils.data import getTrainTripleBatch, getNegRatings
import Yeah_Rec.utils.loss as loss
...
def evaluateRec(FLAGS, model, eval_iter, eval_dict, all_dicts, i_map, logger,triple_train_list = None, eval_descending=True, is_report=False,):
    # Evaluate
    total_batches = len(eval_iter)
    # processing bar
    pbar = tqdm(total=total_batches)
    pbar.set_description("Rec Run Eval")
    ...
    for u_ids in eval_iter:
        ...
        # batch * item
        scores = model.evaluateRec(u_var, all_i_ids=all_i_var, triple_train_list=triple_train_list)
...
def evaluateKG(FLAGS, model, eval_head_iter, eval_tail_iter, eval_head_dict, eval_tail_dict, all_head_dicts, all_tail_dicts, e_map, logger, eval_descending=True, is_report=False):
    ...
    # processing bar
    pbar = tqdm(total=total_batches)
    pbar.set_description("KG Run Eval")
...
def train_loop(FLAGS, model, trainer, rating_train_dataset, triple_train_dataset, rating_eval_datasets, triple_eval_datasets, e_map, i_map, ikg_map, logger, vis=None, is_report=False):
    ...
    model.train()
    model.enable_grad()
    for _ in range(trainer.step, FLAGS.training_steps):     # 默认training_steps=1400000
        ...
        if trainer.step % FLAGS.eval_interval_steps == 0 and (len(rating_eval_datasets) > 0 or len(triple_eval_datasets)>0):
            ...
            for i, eval_data in enumerate(rating_eval_datasets):
                ...
                rec_performances.append( evaluateRec(FLAGS, model, eval_data[0], eval_data[3], all_eval_dicts, i_map, logger, triple_train_list = triple_train_list, eval_descending=True if trainer.model_target == 1 else False, is_report=is_report))
        ...
        if trainer.step % 10 < step_to_switch :
            ...
            if FLAGS.model_type in ["yeah", "jtransup"]:
        ...
        nn.utils.clip_grad_norm_([param for name, param in model.named_parameters()], FLAGS.clipping_max_value)
...
def run(only_forward=False):
    ...
    rating_train_dataset, rating_eval_datasets, u_map, i_map, triple_train_dataset, triple_eval_datasets, e_map, r_map, p, ikg_map = load_data(dataset_path, rec_eval_files, kg_eval_files, FLAGS.batch_size, negtive_samples=FLAGS.negtive_samples, logger=logger)
