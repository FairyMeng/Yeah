import os
import numpy as np
from Yeah_Rec.utils.data import MakeTrainIterator, MakeEvalIterator
...
def loadTriples(filename):
    with open(filename, 'r', encoding='utf-8') as fin:
        ...
        for line in fin:
            ...
            tmp_heads.add(h_id)
            triple_head_dict[(t_id, r_id)] = tmp_heads  # {(t_id,r_id,):h_id}
...
def load_data(kg_path, eval_filenames, batch_size, negtive_samples=1, logger=None):
    ...
    for eval_file in eval_files:
        eval_dataset.append( loadTriples(eval_file) )   # triple_total, triple_list, triple_head_dict, triple_tail_dict
    ...
    new_eval_datasets = []
    dt = np.dtype('int,int')
    for eval_data in eval_dataset:  # 制作head和tail迭代
    ...
    p = {}
    # new_eval_dataset内有6个list:list[0]:tmp_head_iter;list[1]tmp_tail_iter;list[2]:eval_data长度;list[3]:整个eval_triple;list[4]：验证头；list[5]：验证尾
    return train_dataset, new_eval_datasets, e_map, r_map, p
