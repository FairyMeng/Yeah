...
class jTransUPModel(nn.Module):
    ...
    def evaluateRec(self, u_ids, all_i_ids=None,triple_train_list = None):
        ...
        if triple_train_list is not None:
            p_dict = self.e2trID(triple_train_list,e_ids)
            p_e = torch.FloatTensor(p_dict)
            # torch.save([p_dict,p_e,i_e,e_e], "/Users/mengyan/Documents/code/KTUP/log/myTensor.pt")
            p_e = p_e.reshape(item_total, 1)
            p_e = p_e.expand(batch_size,item_total,dim)
            ie_e = i_e + e_e * p_e
